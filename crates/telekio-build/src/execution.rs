    ...
    F: FnOnce(&Context) -> R,
{
    move |context| {
        ...
        let previous = context.rng.replace((current.rng_active != 0).then(|| {
            FastRand::from_telekio(current.rng_one, current.rng_two)
        }));
        let _reset = Reset {
            context,
            state,
    ...
    F: FnOnce(&Context) -> R,
{
    move |context| {
        ...
        let previous = context.current_task_id.replace(
            (current.task_id != 0)
                .then(|| crate::runtime::task::Id::from_telekio(current.task_id)),
        );
        let _reset = Reset {
            context,
            state,
