use super::*;
use crate::runtime::{TaskHooks, task::telekio::HostTaskHooks};
use std::sync::Arc;
#[cfg(not(test))]
use std::{
    ffi::c_void,
...
impl Builder {
    ...
    fn build_host(
        &self,
        local: bool,
    ) -> io::Result<(::telekio::Runtime, Arc<HostTaskHooks>)> {
        let flavor = if local {
        ...
        let config = ::telekio::RuntimeConfig {
            ...
            unhandled_panic: {
                #[cfg(tokio_unstable)]
                {
                    matches!(self.unhandled_panic, UnhandledPanic::ShutdownRuntime).into()
                }
