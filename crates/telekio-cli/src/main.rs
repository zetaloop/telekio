...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(role) = info.role() {
        ...
        return if selected {
            exit(cargo_status(&cargo, &interpreted, Some(&config))?)
        } else {
            exit(cargo_status(&cargo, &arguments, None)?)
        };
    }
...
fn run_groups(
    ...
    for (role, packages) in [(Role::Host, host), (Role::Guest, guest)] {
        ...
        let status = cargo_status(cargo, &arguments, selected.then_some(config.as_path()))?;
        if !status.success() {
...
fn project_info(
    ...
    let metadata = metadata(cargo, arguments, manifest, None, true)?;
...
fn patch_selected(
    ...
    let metadata = metadata(cargo, arguments, manifest, Some(config), false)?;
    let packages = metadata["packages"]
        .as_array()
        .ok_or("Cargo metadata has no packages")?;
...
fn package_selects_workspace(operation: &str) -> bool {
    operation != "install"
        && matches!(
            ...
            CommandKind::Packages | CommandKind::Single
        )
}
