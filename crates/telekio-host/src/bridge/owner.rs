...
use telekio_abi::{
    AttachResult, CallResult, GuestCall, RawAttachment, RawHandle, Shutdown, Waker,
};

use super::runtime::{HandleContext, RuntimeOwner, handle_context, raw_handle};
use super::{host_callback, host_panic};

/// Plugin entry generated by `telekio_abi::plugin!()`.
pub type Attach = unsafe extern "C" fn(RawHandle) -> AttachResult;

/// One loaded plugin's connection to a Tokio runtime.
#[must_use = "the attachment must be detached before unloading its plugin"]
...
/// Attaches a plugin to the current Tokio runtime.
///
/// # Safety
///
...
impl Attachment {
    ...
    /// Stops this plugin's work and releases its attachment before unloading.
    pub async fn detach(&mut self) -> io::Result<()> {
        if self.raw.is_none() {
            return Err(io::Error::other("Telekio attachment is already detached"));
        }
        if ACTIVE_OWNER.get() == Arc::as_ptr(&self.owner) {
            return Err(io::Error::other(
                "plugin detachment cannot run inside one of its guest callbacks",
            ));
        }
        shutdown_owner(&self.owner).await?;
        let raw = self.raw.take().expect("Telekio attachment is detached");
...
impl OwnerState {
    ...
    fn runtimes(&self) -> Vec<Arc<RuntimeOwner>> {
        self.state
            .lock()
            .unwrap()
            .runtimes
