...
use {
    ...
    telekio::{OperationPoll, OwnedBytes, Poll, Status, Waker},
};
...
pub(super) unsafe extern "C" fn start(context: *const c_void) -> DumpResult {
    ...
    match catch_unwind(AssertUnwindSafe(|| create(context))) {
...
fn create(context: &HandleContext) -> Result<DumpOperation, String> {
...
fn create(_: &HandleContext) -> Result<DumpOperation, String> {
...
unsafe extern "C" fn release(data: *mut c_void) -> CallResult {
    super::host_callback(|| unsafe { &*data.cast::<HostResource<Dump>>() }.release())
}
